...
document.addEventListener("keydown", function(e1) {
    if (e1.key === "Escape" && !modal.classList.contains("hidden")) closeModal();
});
///////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////
// Button scrolling
btnScrollTo.addEventListener("click", function(e1) {
    const s1coords = section1.getBoundingClientRect();
    console.log(s1coords);
    console.log(e1.target.getBoundingClientRect());
...
*/ document.querySelector(".nav__links").addEventListener("click", function(e1) {
    e1.preventDefault();
    //console.log(e.target); the place where the event actually happend.
    //Matching strategy:
    if (e1.target.classList.contains("nav__link")) {
        const id = e1.target.getAttribute("href");
        console.log(id);
...
tabsContainer.addEventListener("click", function(e1) {
    const clicked = e1.target.closest(".operations__tab");
...
------------------------------------------------------------------------------------------------------------------------------------------------
this is an another modern feature using the intersection observer API. And this time we are gonna reveal elements as we scroll close to them. And this effect can give your pages a very nice touch. And you can infact easily implement it without any external library.*/ const allSections = document.querySelectorAll(".section");
...
const slider = function() {
    ...
    const createDots = function() {
        slides.forEach(function(_, i) {
            dotContainer.insertAdjacentHTML("beforeend", `<button class="dots__dot dots__dot--active" data-slide="${i}"></button>`); //Each dot is gonna be one element with the class of "dots dot". And then it will have this 'data attribute of slide' "with the number of the slide" that clicking the button will go to. So, once again the "data attribute" 'holds some data that we need in order to make the functionality work'.
...
*/ document.addEventListener("keydown", function(e1) {
        if (e1.key === "ArrowLeft") prevSlide(); //this is also a reason why we created a separate function for this.
        e1.key === "ArrowRight" && nextSlide(); //Short circuiting in JavaScript (and other languages) is when only part of a boolean expression is evaluated in order to get the result of the entire expression.
    });
    //let's now make our dots work:
    dotContainer.addEventListener("click", function(e1) {
        if (e1.target.classList.contains("dots__dot")) {
            const { slide } = e1.target.dataset; //and now for make it even better we can now use destructuring because the name of the variable is exactly same as the source of the data-slide element and so we are reading this form the entry object and so we can simply destructure it.
...
message.textContent = "We use cookies for improved functionality and analystics."; //so this inserts simply text but of course we can also insert a HTML.
message.innerHTML = 'We use cookies for improved functionality and analystics. <button class="btn btn--close-cookie">Got it!</button>'; //and remeber we can use both of these properties to 'read' and to 'set content'.
...
*/ document.addEventListener("DOMContentLoaded", function(e1) {
    ...
    console.log("HTML parsed and DOM tree built!", e1); //down in the network part of the browser you can actually see the time that it takes for the event to be fired.
});
...
*/ window.addEventListener("load", function(e1) {
    console.log("Page fully loaded", e1);
